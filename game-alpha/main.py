    ...
    def load_map(self, map : utils.Map):
        ...
        for row, tiles in enumerate(map.data):
            for col, tile in enumerate(tiles):
                ...
                if tile == "r": #Gate (Red)
                    _ = Gate(self, col, row, self.redGateImage, settings.ITEM_TYPE_KEY_RED, "red")
                if tile == "y": #Gate (Yellow)
                    _ = Gate(self, col, row, self.yellowGateImage, settings.ITEM_TYPE_KEY_YELLOW, "yellow")
                if tile == "o": #Gate (Orange)
                    _ = Gate(self, col, row, self.orangeGateImage, settings.ITEM_TYPE_KEY_ORANGE, "orange")
                if tile == "b": #Gate (Blue)
                    _ = Gate(self, col, row, self.blackGateImage, settings.ITEM_TYPE_KEY_BLACK, "blue")
                if tile == "p": #Gate (Purple)
                    _ = Gate(self, col, row, self.purpleGateImage, settings.ITEM_TYPE_KEY_PURPLE, "purple")
                ...
                if tile == "E": #Enemy (Bouncer)
                    m = Mob(self, 32, 32, col*settings.TILE_SIZE, row*settings.TILE_SIZE, settings.KING_CRIMSON, 3, 10,)
                    self.mob_sprites.add(m)
                    self.all_sprites.add(m)
                if tile == "s": #Enemy (Speedy)
                    m = Mob(self, 32, 32, col*settings.TILE_SIZE, row*settings.TILE_SIZE, settings.GREEN, 5, 5, followPlayer_bool=True)
                    self.mob_sprites.add(m)
                    self.all_sprites.add(m)
